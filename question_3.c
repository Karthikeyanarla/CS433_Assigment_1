...
// helperFunctions
// fetch pid using source portNumber
uint32_t fetchPID(uint16_t portNumber)
{
    ...
    snprintf(command, sizeof(command), "sudo netstat -ltnup | grep ':%d ' | awk '{print $7}' | cut -d'/' -f1", portNumber);
...
void packet_handler(FlowTable *table, unsigned char *packet, int packet_length)
{
    ...
    uint32_t processid = fetchPID(source_port);
...
Flow *search_flow_by_source_port(FlowTable *table, uint16_t port, char *ip)
{

    for (int i = 0; i < table->size; i++)
    {
        if (table->flows[i].source == inet_addr(ip))
            if (table->flows[i].source_port == port)
                return &table->flows[i];
            else if (table->flows[i].destination == inet_addr(ip))
                if (table->flows[i].destination_port == port)
                    return &table->flows[i];
    }
    return NULL;
}
...
int main()
{
    ...
    while (1)
    {
        ...
        Flow *flow = search_flow_by_source_port(table, port_str, ip);
        if (flow == NULL)
        {
